...
export default function Edit({ blog }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    ...
    format_type: blog.format_type || 'markdown',
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(
    blog.image_path ? `/storage/${blog.image_path}` : null
  );
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImagePreview(`/storage/${blog.image_path}`);
    } else {
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <Link href={`/blogs/${blog.id}`} className="flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors mb-4">
              <ArrowLeftIcon className="h-5 w-5 mr-2" />
              Back to Blog
            </Link>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Blog Post</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-2">Update your blog post content and settings</p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
                ...
                {imagePreview && (
                  <div className="relative max-w-md">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setImagePreview(blog.image_path ? `/storage/${blog.image_path}` : null);
                        setData('image', null);
            ...
            <div className="space-y-2">
              ...
              <MarkdownEditor
                ...
                placeholder="Edit your blog content here. You can use Markdown formatting, including code blocks, tables, and math equations."
              />
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-end border-t border-gray-200 dark:border-gray-700 pt-6">
              ...
              >
                {processing ? (
                  ...
                  data.status === 'published' ? 'Update & Publish' : 'Save Changes'
                )}
              </Button>
            </div>
          </form>
